...
# Demo ledger transactions for CNPG — mirrors src/ledger/ledger-db/initdb/1_create_transactions.sh
# (embedded ledger-db runs this when USE_DEMO_DATA=True).
...
# 3 pay periods x 4 demo users = 12 payroll deposits from External Bank.
DEMO_DEPOSITS=$(
  PGPASSWORD="$POSTGRES_PASSWORD" psql -X -v ON_ERROR_STOP=1 \
    --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -t -A <<-EOSQL
    SELECT COUNT(*) FROM transactions
    WHERE from_acct = '9099791699'
      AND amount = 250000
      AND to_acct IN ('1011226111', '1033623433', '1055757655', '1077441377');
EOSQL
)

if [[ "${DEMO_DEPOSITS}" -ge 12 ]]; then
  echo "demo ledger transactions already seeded (${DEMO_DEPOSITS} payroll deposits); skipping"
  exit 0
fi
...
create_transactions
echo "demo ledger transactions seeded"
